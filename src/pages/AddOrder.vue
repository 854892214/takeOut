    ...
    <div class="integral-keyboard" v-if="this.$store.getters.openMembershipSystem=='TRUE'&&this.$store.getters.isStoreUser=='TRUE'">
      <table>
        ...
        <tr>
          ...
          <!-- class="fobid"-->
          <td @click="integralKeyBordNum('K')" data-code="K" rowspan="2">Ok</td>
        </tr>
        <tr>
    ...
    methods: {
      ...
      payment(orderNum, tip, paytype) {
        ...
        if (this.$store.getters.isStoreUser == "TRUE") {
          ...
          body = {
            deduction_integral: this.integralInputVal === '' || this.integralInputVal == null ? 0 : this.integralInputVal,
            num: orderNum,
            tip: tip
          }
        } else {
      ...
      alertConfirm() {
        ...
        if (inputVal < maxDeductionIntegral) {
          ...
          let totalRound = +(this.totalAmount.toFixed(2).split(".")[1])
          if (totalRound > 0) {
            this.canDeductionAmount = this.transPrice(maxDeductionIntegral)
